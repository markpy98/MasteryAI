...
import { analyzeThesisPdf, analyzeTranscript } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  const handleVideoAnalysis = async (transcript: string, title: string) => {
    setAppState(AppState.ANALYZING);
    setErrorMsg(null);
    try {
      const data = await analyzeTranscript(transcript, title);
      const savedDoc = saveAnalysis(data, selectedFolderId, title); 
      refreshData();
      setTimeout(() => {
        setSelectedDocId(savedDoc.id);
        setAppState(AppState.SUCCESS);
      ...
      console.error(error);
      setAppState(AppState.ERROR);
      setErrorMsg("Ocurrió un error al analizar la transcripción. Por favor intenta de nuevo.");
    }
  };
                ...
                <div className="animate-fade-in py-8">
                  ...
                  <div className="bg-white p-6 md:p-10 rounded-2xl shadow-xl shadow-slate-200/50 mb-8 border border-slate-100">
                    <FileUpload 
                      ...
                      onJsonLoad={() => fileInputRef.current?.click()}
                      disabled={false} 
                    />
